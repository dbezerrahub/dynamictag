...
class simple_html_dom_node
{
	 ...
	 * @param String $str
	 * @param Boolean $concat: Indica se o conteúdo será substituido ou concatenado
	 */
	...
	if (is_array($content)) {
		foreach ($content as $cont) {
			if (is_object($cont)) {
				if (strpos(get_class($cont), 'Dob\DynamicTag') !== false) {
					$cont = "\n" . $cont->toStr() . "\n";
				}
			}
			if (!$reset) {
		...
		if (is_object($content)) {
			if (strpos(get_class($content), 'Dob\DynamicTag') !== false) {
				$content = "\n" . $content->toStr() . "\n";
			}
		}
		if (!$reset) {
