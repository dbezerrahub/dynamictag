 ...
 */
function setIn($content, $reset = false) {
//$this->innertext = str_replace("\n", '', $this->innertext);
if (is_array($content)) {
    foreach ($content as $cont) {
        if (is_object($cont)) {
            if (strpos(get_class($cont), 'Dob\DTag') !== false) {
                $cont = "\n" . $cont->toStr() . "\n";
            }
        }
        if (!$reset) {
    ...
    if (is_object($content)) {
        if (strpos(get_class($content), 'Dob\DTag') !== false) {
            $content = "\n" . $content->toStr() . "\n";
        }
    }
    if (!$reset) {
...
if (is_null($idx))
    ...
    $idx = count($found) + $idx;
$rt = (isset($found[$idx])) ? $found[$idx] : null;
if (!is_null($rt)) {
    //$rt->setIn("\n");
}
return $rt;
 ...
 * @param type $footer
 */
function __construct($css = true, $js = false, $header = false, $footer = false) {
@header('Content-Type: text/html; charset=utf-8');
$this->dirView = searchDirView(get_class($this), resource_path() . '/views');
$this->css = $css;
$this->js = $js;
$this->htmlFile = file_get_html($this->dirView . '/html/' . lcfirst(get_class($this)) . '.html');
...
/**
 * Carrega um arquivo css específico para a view
 */
...
/**
 * Carrega um arquivo js específico para a view
 */
function pushJs($js) {
$this->pushJs[] = $js;
}
...
/**
 * Insere em todas as views os arquivos css informados no array
 * $glbCss
 * @return string
 */
function getGlobalCss() {
$glbCss = config('view.globalCss');
$rt = "";
foreach ($glbCss as $cssLink) {
    if (substr($cssLink, 0, 4) === "http") {
        $rt .= "<link rel='stylesheet' href='$cssLink'>\n";
    } else {
        $rt .= "<link rel='stylesheet' href='" . url('public/css/' . $cssLink) . "'>\n";
    }
}
return $rt;
}
...
/**
 * Insere em todas as views os arquivos js informados no array
 * $glbJs
 * @return string
 */
function getGlobalJs() {
$glbJs = config('view.globalJs');
$rt = "";
foreach ($glbJs as $jsLink) {
    if (substr($jsLink, 0, 4) === "http") {
        $rt .= "<script src='$jsLink' type='text/javascript'></script>\n";
    } else {
        $rt .= "<script src='" . url('public/js/' . $jsLink) . "' type='text/javascript'></script>\n";
    }
}
return $rt;
}
...
function getDirCss($dir) {
try {
    $cssName = lcfirst(get_class($this));
    $d = new \DirectoryIterator($dir);
    $directories = null;
    $rs = false;
    foreach ($d as $item) {
        ...
        if ($item->isFile()) {
            if (strpos($item->getFilename(), '.css') !== false) {
                $css = str_replace('.css', '', $item);
                if ($css == $cssName) {
                    return $item->getPath() . '/' . $cssName . '.css';
                }
            }
        } else {
            $directories[] = $item->getPath() . '/' . $item;
...
function getDirJs($dir) {
try {
    $jsName = lcfirst(get_class($this));
    $d = new \DirectoryIterator($dir);
    $directories = null;
    $rs = false;
    foreach ($d as $item) {
        ...
        if ($item->isFile()) {
            if (strpos($item->getFilename(), '.js') !== false) {

                $js = str_replace('.js', '', $item);
                if ($js == $jsName) {
                    return $item->getPath() . '/' . $jsName . '.js';
                }
            }
        } else {
            $directories[] = $item->getPath() . '/' . $item;
...
return $this->htmlFile->find($tag . "[$att=$value]", 0);
}
...
$htmlView = '';
if ($this->htmlFile) {
    $htmlView = $this->htmlFile->show();
}

// Busca o css
$cssView = '';
if ($this->css) {
    $css = file_get_html($this->getDirCss(resource_path('views')));
    if ($css) {
        //$cssView = '<style>'.preg_replace('/\s+/', '', $css->show()).'</style>';
        $cssView = '<style>' . $css->show() . '</style>';
        $css->clear();
    }
}
// Busca o Js
$jsView = '';
if ($this->js) {
    $js = file_get_html($this->getDirJs(resource_path('views')));
    if ($js) {
        //$jsView = '<script type="text/javascript">'.preg_replace('/\s+/', '', $js->show())."</script>\n";
        $jsView = '<script type="text/javascript">' . $js->show() . "</script>\n";
        $js->clear();
    }
}
...
// Css Globais
$show .= $this->getGlobalCss();
// Css avulsos
foreach ($this->pushCss as $pcss) {
    if (substr($pcss, 0, 4) === "http") {
        $show .= "<link rel='stylesheet' href='$pcss'>\n";
    } else {
        $css = file_get_html($pcss);
        $show .= '<style>' . $css->show() . "</style>\n";
        $css->clear();
    }
}
// Css padrão
...
// Js Globais
//$show .= "<script type='text/javascript'>globalDtagUrl = ".url()."</script>\n";
$show .= $this->getGlobalJs();
// Js avulsos
foreach ($this->pushJs as $pjs) {
    if (substr($pjs, 0, 4) === "http") {
        $show .= "<script src='$pjs' type='text/javascript'></script>\n";
    } else {
        $js = file_get_html($pjs);
        $show .= "<script type='text/javascript'>$js</script>\n";
        $js->clear();
    }
}
...
$show .= $jsView;
// Js Dtag
$show .= "<script src='" . url('dtag/js') . "' type='text/javascript'></script>\n";
